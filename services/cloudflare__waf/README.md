...
- `BlockIP` accepts validated IPv4, IPv6, and CIDR targets and calls `POST /…/firewall/access_rules/rules`. Duplicate targets in one request are processed once, and concurrent creation of the same scope, target, and mode is serialized within the service process before rechecking upstream. If a later create fails, the gRPC error includes `partial_created_ids` for reconciliation or safe whole-request retry.
